...
@app.route('/api/create-test-order', methods=['POST'])
@jwt_required()
def create_test_order():
    """Create a $0 order for the test product to test reviews"""
    try:
        ...
        # Get test product
        product = mongo.db.products.find_one({'slug': 'review-test-product'})
        if not product:
            # Create test product if it doesn't exist
            product_id = init_test_product()
            if not product_id:
                return jsonify({'success': False, 'message': 'Failed to create test product'}), 500
        ...
        mongo.db.review_requests.insert_one(review_request)
        
        # Send review request email
        review_link = f"{request.host_url.rstrip('/')}/api/reviews/request/{review_token}"
        subject = f"Review Request for Your Test Order"
        body = f"""Hi {user['name']},

Thank you for creating a test order. To help us test the review system, please click the link below to leave a review:
...
def init_test_product():
    """Initialize a $0 test product for the review system"""
    try:
        test_product = {
            'name': 'Review Test Product',
            'slug': 'review-test-product',
            'description': 'This is a test product for the review system. Price is $0.',
            'price': 0.00,
            'stock_quantity': 999,
            'image': 'static/images/rose-water-lotion.jpg',  # Using existing image
            'created_at': datetime.utcnow()
        }
        result = mongo.db.products.insert_one(test_product)
